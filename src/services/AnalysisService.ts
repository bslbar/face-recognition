import {
  AzureImageAnalyzer,
  AzureStorageProcessor,
  GoogleImageAnalyzer,
  IAzureEmotions,
  IGoogleEmotions,
  IStorageProcessorItem
} from "../modules";
...
export class AnalysisService {
  ...
  public async azureWork(): Promise<any> {
    return new Promise<any>(async (resolve, reject) => {
      ...
      interval(3300)
        .pipe(take(images.length))
        .subscribe(
          async index => {
            let item = images[index];
            console.log(`Start analyze images ${item.name}...`);

            const faces = await this.azureImageAnalyzer.analyzeImage(
              item.publicUri
            );
            if (faces.length > 0) {
              faces.forEach(face => {
            ...
            console.log(
              `analyze AZURE ${item.name}, processed ${this.getPercentage(
                index,
                images.length
              )}%`
            );
  ...
  public async googleWork(): Promise<any> {
    ...
    for (let i = 0; i < images.length; i++) {
      ...
      const faces = await this.googleImageAnalyzer.analyzeImage(
        images[i].publicUri
      );
      if (faces.length > 0) {
        faces.forEach(face => {
          output.push({
            ...
            angerLikelihood: this.googleImageAnalyzer.transcodingEmotionsValue(
              face.angerLikelihood
            ),
            joyLikelihood: this.googleImageAnalyzer.transcodingEmotionsValue(
              face.joyLikelihood
            ),
            sorrowLikelihood: this.googleImageAnalyzer.transcodingEmotionsValue(
              face.sorrowLikelihood
            ),
            surpriseLikelihood: this.googleImageAnalyzer.transcodingEmotionsValue(
      ...
      console.log(
        `analyzed GOOGLE ${images[i].name}, processed ${this.getPercentage(
          i,
          images.length
        )}%`
      );
